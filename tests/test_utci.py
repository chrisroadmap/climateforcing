...
def test_utci_array():
    ...
    TEST_RESULT = utci(np.array([295,296]), np.array([303,304]), np.array([6.0,6.0]), np.array([100,100]))
...
def test_mrt_list():
    EXPECTED_RESULT = np.array([313.33875095746555, 291.2519186818613])
    TEST_RESULT = mean_radiant_temperature([150,50], [350,150], [400,200], [100,50], [700,400], cos_zenith=[0.5,0.2])
    assert np.allclose(TEST_RESULT, EXPECTED_RESULT)
...
def test_integration_array():
    ...
    TEST_RESULT = utci(np.array([295,296]), mrt, np.array([6.0,6.0]), np.array([100,100]))
    assert np.allclose(TEST_RESULT, EXPECTED_RESULT)

def test_integration_list():
    EXPECTED_RESULT = np.array([22.33807826, 18.04481664])
    mrt = mean_radiant_temperature([150,50], [350,150], [400,200], [100,50], [700,400], cos_zenith=[0.5,0.2])
