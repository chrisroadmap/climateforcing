import numpy as np
from climateforcing.atmos.humidity import specific_to_relative, relative_to_specific
...
def test_relative_to_specific():
    ...
    test_result = relative_to_specific(50, air_temperature=290, pressure=1e5, rh_percent=True)
    assert test_result == EXPECTED_RESULT
...
def test_specific_to_relative():
    ...
    assert test_result == EXPECTED_RESULT
    test_result = specific_to_relative(0.006, air_temperature=290, pressure=1e5, rh_percent=True)
